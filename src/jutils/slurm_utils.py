...
def slurm_wrapper(args, save_dir, func, func_kwargs, resubmit=True, func_args=()):
    ...
    job_name = (
        args.sl_name if args.sl_name is not None else "/".join(save_dir.split("/")[-2:])
    )
...
def add_slurm_args(arg_parser):
    ...
    arg_parser.add_argument(
        "--sl_part", default="abhinav,shubham,abhinavlong,shubhamlong", type=str
    )
    return arg_parser
